...
def start_pipeline(webhook_payload: str = None, scenario_index: int = None) -> str:
    print('📡 Fetching SAP Telemetry...')
    from src.ai.decision_engine import analyze_telemetry_and_decide
    from src.ai.memory_store import save_memory, get_memory, save_remediation
    from src.monitor.log_reader import generate_log_entry as gen_log
    
    from src.api.ws_manager import trigger_dashboard_event
    
    # Step 1: Generate/fetch the error log
    if scenario_index is not None:
        log_entry = gen_log(scenario_index)
        raw_telemetry = json.dumps(log_entry)
        trigger_dashboard_event("INCOMING_ERROR", data={"message": "Fetched live telemetry via API", "payload": log_entry})
    elif webhook_payload:
        raw_telemetry = webhook_payload
        try:
            log_entry = json.loads(webhook_payload)
        except Exception:
            log_entry = {"raw_log": webhook_payload, "type": "UNKNOWN"}
    else:
        log_entry = gen_log()  # random scenario
        raw_telemetry = json.dumps(log_entry)
        trigger_dashboard_event("INCOMING_ERROR", data={"message": "Fetched live telemetry via API", "payload": log_entry})
    
    error_json = json.dumps(log_entry) if isinstance(log_entry, dict) else str(log_entry)
    
    # Persist the error
    save_memory("last_error", error_json)
    
    # Step 2: LLM Decision Engine triage
    error_type = log_entry.get("type", "UNKNOWN") if isinstance(log_entry, dict) else "UNKNOWN"
    error_subtype = log_entry.get("subtype", "N/A") if isinstance(log_entry, dict) else "N/A"
    
    print(f'🧠 Decision Engine Analyzing {error_type}: {error_subtype}...')
    trigger_dashboard_event("AGENT_DECISION", data={
        "message": f"Decision Engine starting triage for {error_type} error."
    })
    
    should_run, reason = analyze_telemetry_and_decide(error_json)
    
    if not should_run:
        print(f'🟢 Decision: {reason}. System is stable.')
        save_remediation({"status": "Escalated", "reason": reason})
        trigger_dashboard_event("INFO", data={"message": f"Pipeline Skipped: {reason}"})
        return json.dumps({"status": "SKIPPED", "reason": reason})
    
    # Step 3: Route to the real deterministic agent pipeline
    print(f'🚨 Decision: {reason}. Triggering Autonomous Pipeline...')
    trigger_dashboard_event("AGENT_DECISION", data={
        "message": f"Agent determined: {reason}. Routing to Autonomy."
    })
    trigger_dashboard_event("INFO", data={
        "message": "Agent started - Multi-Agent Task Orchestration."
    })
    
    # Use the REAL local pipeline (not CrewAI hallucination)
    pipeline_result = run_self_healing_pipeline(log_entry=log_entry)
    
    resolved = pipeline_result.get("resolved", False)
    
    # Broadcast each step to the dashboard
    for step in pipeline_result.get("steps", []):
        step_type = "EXECUTION_SUCCESS" if "✅" in step.get("status", "") else "EXECUTION_FAILED"
        trigger_dashboard_event(step_type, data={
            "message": f"[{step['agent']}] {step['action']}",
            "status": step["status"],
    ...
    # Persist remediation
    try:
        save_remediation({"payload": json.dumps(pipeline_result.get("execution", {}))})
    except Exception:
        save_memory("last_remediation", json.dumps(pipeline_result.get("execution", {})))
    
    # Final summary
    summary = pipeline_result.get("summary", "Pipeline complete.")
    print(f'\n{"="*50}')
    print(f'{"✅" if resolved else "❌"} {summary}')
    print(f'{"="*50}')
    ...
    return json.dumps({
        ...
        "summary": summary,
        "diagnosis": {
            "root_cause": pipeline_result.get("diagnosis", {}).get("root_cause"),
            "severity": pipeline_result.get("diagnosis", {}).get("severity"),
            "human_readable": pipeline_result.get("diagnosis", {}).get("human_readable"),
        },
        "plan": pipeline_result.get("plan", {}),
        "execution": {
            "governance_check": pipeline_result.get("execution", {}).get("governance_check"),
            "bapi_name": pipeline_result.get("execution", {}).get("bapi_name"),
            "fix_applied": pipeline_result.get("execution", {}).get("fix_applied"),
            "bapi_result": pipeline_result.get("execution", {}).get("bapi_result"),
        },
    })
